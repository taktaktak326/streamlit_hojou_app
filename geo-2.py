...
if st.button("処理を開始"):
    ...
    if uploaded_pori_files and uploaded_nouchi_files:
        ...
        # 圃場名を同じ筆ポリゴンごとにまとめる
        if "住所地番" not in result.columns:
            st.error("⚠️ 結合結果に '住所地番' カラムが存在しません。")
            st.write(result.columns)
            st.stop()

        # 圃場名を筆ポリゴン単位でグループ化（カンマ区切り）
        grouped = result.groupby('geometry').agg({
            '住所地番': lambda x: ', '.join(sorted(set(x)))
        }).reset_index()

        # GeoDataFrameとして再構成
        gdf_grouped = gpd.GeoDataFrame(grouped, geometry='geometry', crs=result.crs)
        gdf_grouped = gdf_grouped.rename(columns={'住所地番': '圃場一覧'})

        # 表示
        st.subheader("📋 同じ筆ポリゴン内の圃場一覧（カンマ区切り）")
        st.write(gdf_grouped.head())

        # 地図プロット（圃場一覧付き）
        st.subheader("📍 筆ポリゴン + 圃場名一覧マップ")
        if not gdf_grouped.empty:
            centroid = gdf_grouped.geometry.centroid.iloc[0]
        ...
        if uploaded_excel_file and sheet_name:
            try:
                ...
                def find_matching_geometry(address):
                    ...
                    if not match.empty and 'geometry' in match.columns:
                        geometries = match['geometry'].values
                ...
                # **Excelファイルをダウンロード（2シート構成）**
                output_buffer = BytesIO()
                with pd.ExcelWriter(output_buffer, engine="xlsxwriter") as writer:
                    # 圃場単位のデータ（元Excel + geometry付き）
                    df_excel.to_excel(writer, sheet_name="MatchedData", index=False)

                    # 筆単位で圃場をまとめたデータ
                    gdf_grouped[["圃場一覧", "geometry"]].to_excel(writer, sheet_name="PolygonGrouped", index=False)

                output_buffer.seek(0)

                st.download_button(
                    label="📥 更新済みExcel（2シート）をダウンロード",
                    data=output_buffer,
                    file_name=f"{sheet_name}_圃場地番と筆一覧ファイル.xlsx",
